...
from .exceptions import CurrencyMismatch, ExchangeRateNotFound, InvalidOperandType
...
class Money:
    ...
    def __init__(self, amount="0", currency=None):
        ...
        if not REGEX_CURRENCY_CODE.match(currency):
            raise ValueError(
                "currency not in ISO 4217 format: " "'{}'".format(currency)
            )
    ...
    def __divmod__(self, other):
        if isinstance(other, Money):
            ...
            return (
                self.__class__(whole, self._currency),
    ...
    def format(
        ...
        if BABEL_AVAILABLE:
            ...
            raise NotImplementedError(
                "formatting requires Babel " "(https://pypi.python.org/pypi/Babel)"
            )
    ...
    def loads(cls, s):
        ...
        try:
            ...
            # RADAR: Python2
            money.six.raise_from(
                ValueError("failed to parse string " " '{}': {}".format(s, err)), None
            )
